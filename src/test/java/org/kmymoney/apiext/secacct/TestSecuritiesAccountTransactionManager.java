...
public class TestSecuritiesAccountTransactionManager {
	...
	private void test01_check_persisted_ml(File outFile) throws Exception {
		...
		assertEquals(0, genTrx.getBalanceRat().getNumerator().longValue());
		assertEquals(1, genTrx.getBalanceRat().getDenominator().longValue());
		// .
		...
		assertEquals(BUY_EXP_1_FP, feeTax);
	...
	private void test02_check_persisted_ml(File outFile) throws Exception {
    	...
    	FixedPointNumber expensesSumFP = FixedPointNumber.ZERO.copy();
    	BigFraction      expensesSumBF = BigFraction.ZERO;
    	for ( AcctIDAmountFPPair elt : EXPENSES_ACCT_AMT_FP_LIST ) {
    	    expensesSumFP.add(elt.amount()); // mutable
    	}
    	for ( AcctIDAmountBFPair elt : EXPENSES_ACCT_AMT_BF_LIST ) {
    	    expensesSumBF = expensesSumBF.add(elt.amount()); // immutable
    	}
    	FixedPointNumber divNetFP = DIV_GROSS_FP.copy().subtract(expensesSumFP);
    	BigFraction      divNetBF = DIV_GROSS_BF.subtract(expensesSumBF);
		...
		assertEquals(null, splt2.getAction());
		// .
		assertEquals(divNetFP, splt2.getShares());
		assertEquals(divNetBF, splt2.getSharesRat());
		// .
		assertEquals(divNetFP, splt2.getValue());
		assertEquals(divNetBF, splt2.getValueRat());
		// .
